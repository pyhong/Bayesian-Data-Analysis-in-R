...
const int NBREAK = (NCOEFFS - 2);  // nbreak=ncoeffs-2 since k = 4

//[[Rcpp::export]]
//Data generator for B-spline
Rcpp::List genData()
{
  const size_t n =N;
  size_t i;
  ...
  gsl_rng *r;
  gsl_rng_env_setup();
  r = gsl_rng_alloc(gsl_rng_default); //setup the environment to generata
                                      //random number
...
Rcpp::List fitData(Rcpp::DataFrame ds)
{
  ...
  Rcpp::DataFrame D(ds);  //construct data.frame
  RcppGSL::vector<double> y = D["y"]; //access columns by name
  RcppGSL::vector<double> x = D["x"]; //assigning GSL vectors
  ...
  return(Rcpp::List::create(
            Rcpp::Named("M") = M,
            Rcpp::Named("chisdof") = Rcpp::wrap(chisq/dof),
